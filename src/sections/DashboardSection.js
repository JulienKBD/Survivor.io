...
export default function DashboardSection() {
  ...
  React.useEffect(() => {
    const fetchStartups = async () => {
      try {
        const res = await fetch(`${process.env.NEXT_APP_API_URL}/startups`);
    ...
    const fetchMetrics = async () => {
      try {
        const promises = startupIds.map(id =>
          fetch(`${process.env.NEXT_APP_API_URL}/startups/${id}`).then(res => res.json())
        );
  ...
  React.useEffect(() => {
    const fetchProjects = async () => {
      try {
        const res = await fetch(`${process.env.NEXT_APP_API_URL}/projects`);
